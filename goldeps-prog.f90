...
real(8):: matrixAu(n1,3) , x(nw), eps_Drude(nw), eps_Mar(nw), eps_Gen(nw), epsAur(nw)
!counters
integer:: i,k,j
!integration results
...
eps_Gen(i)=Gen_Plasma(x(i))
end do

100 format(5(f15.3))

do i=1,nw
write(18,100) x(i), epsAur(i), eps_Drude(i), eps_Mar(i), eps_Gen(i)
end do
