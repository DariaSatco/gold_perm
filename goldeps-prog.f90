...
real(8):: mostep_one_osc_re(nw,6), mostep_one_osc_im(nw,6)
real(8):: ld_one_osc_re(nw,6), ld_one_osc_im(nw,6)
real(8):: bb_one_osc_re(nw,6), bb_one_osc_im(nw,6)
...
write(16,20), (matrixAu(i,j),j=1,3)
end do
...
!eps(w)_re.txt
write(25,250) 'freq (eV)', 'Lorentz', 'Gen_Plasma', 'Bren_Borm'

do i=1,nw
write(25,200) x(i), LD_model_re(i)+Lorentz_oscil_re(i), Gen_pl_mod(i)+Mostep_oscil_re(i), BB_mod_re(i)+BB_oscil_re(i)
!write(25,200) x(i), Lorentz_oscil_re(i), Mostep_oscil_re(i), BB_oscil_re(i)
end do

!eps(w)_im.txt
write(26,250) 'freq (eV)', 'Lorentz', 'Gen_Plasma', 'Bren_Borm'

do i=1,nw
write(26,200) x(i), LD_model_im(i)+Lorentz_oscil_im(i), Mostep_oscil_im(i), BB_mod_im(i)+BB_oscil_im(i)
!write(26,200) x(i), Lorentz_oscil_im(i), Mostep_oscil_im(i), BB_oscil_im(i)
end do

!want to look at every oscillator
open(unit=23, file='lorentz_bb_oscillators_re.txt', status='replace')
open(unit=24, file='lorentz_bb_oscillators_im.txt', status='replace')

!real part of oscillators
do i=1,nw
do k=1,6
mostep_one_osc_re(i,k)=oscil_gold_re_im(x(i), gAu(k), gammaAu(k), wAu(k), 1, 0)
ld_one_osc_re(i,k)=oscil_gold_re_im(x(i), force_LD(k), gammaR(k), wR(k), 1, 0)
bb_one_osc_re(i,k)=Brendel_Bormann_oscil_Re_Im(x(i), force_BB(k), gammaRb(k), wRb(k), sigmaRb(k), 1, 0)
end do
end do

!lorentz_bb_oscillators_re.txt
write(23, 250) 'freq (eV)', 'Mostep', 'Lor-Dr', 'Bren-Borm'

do i=1,nw
do k=1,6
write(23, 200) x(i), mostep_one_osc_re(i,k), ld_one_osc_re(i,k), bb_one_osc_re(i,k)
end do
end do

!imaginary part of oscillators
do i=1,nw
do k=1,6
mostep_one_osc_im(i,k)=oscil_gold_re_im(x(i), gAu(k), gammaAu(k), wAu(k), 1, 1)
ld_one_osc_im(i,k)=oscil_gold_re_im(x(i), force_LD(k), gammaR(k), wR(k), 1, 1)
bb_one_osc_im(i,k)=Brendel_Bormann_oscil_Re_Im(x(i), force_BB(k), gammaRb(k), wRb(k), sigmaRb(k), 1, 1)
end do
end do

!lorentz_bb_oscillators_im.txt
write(24, 250) 'freq (eV)', 'Mostep', 'Lor-Dr', 'Bren-Borm'

do i=1,nw
do k=1,6
write(24, 200) x(i), mostep_one_osc_im(i,k), ld_one_osc_im(i,k), bb_one_osc_im(i,k)
end do
end do
